...
func PrintUsage() {
	...
	fmt.Fprintln(os.Stderr, os.Args[0], "<-username USERNAME> <-clientId ID> <-privateKey PATH_TO_PRIVATE_KEY> <-installationId INSTALLATION_ID | -organisation ORGANISATION> [-url GITHUB_API_URL] <get|store|erase>", os.Args[0])
	flag.PrintDefaults()
}
...
func main() {
	...
	flag.StringVar(&app.organisation, "organisation", "", "GitHub App Organisation, required if InstallationId not supplied")
	...
	if app.installationId == 0 {
		// Get InstallationId from the organisation
		if len(app.organisation) == 0 {
			panic("When InstallationId is not supplied, Organisation is required")
		} else {
